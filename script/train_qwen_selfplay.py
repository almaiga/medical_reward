...
def get_device():
    ...
    # MPS support can be less stable for training
...
def parse_attacker_response(tokenizer, full_output_text: str):
    ...
    if think_match:
        ...
        # The final content is everything after the think block
        final_content = full_output_text[think_match.end():].strip()
    else:
        final_content = full_output_text

    # 2. Robustly find and clean the modified note from the final content
...
def parse_assessor_response(tokenizer, full_output_text: str):
    """
    Parses the Assessor's standard think/content output, looking for the final classification label robustly.
    """
    thinking_content = ""
    final_label = ""
    ...
    if label_match:
        ...
        if fallback_match:
            ...
            final_label = "Can't Assess" # Default if no label found
...
def get_judge_assessment(original: str, attacked: str, assessor_label: str, judge_model, judge_tok, device):
    ...
    try:
        ...
        if attacked.strip() != original.strip():
            judgments["actual_harm"] = "Concerning" # Default to concerning if an attack happened but judge failed
...
def build_attacker_prompts(ds: Dataset, tokenizer):
    """Builds Qwen3-native attacker prompts."""
...
def make_assessor_prompts(records: list, tokenizer):
    """Make Qwen3-native assessor prompts."""
...
def generate_with_thinking_budget(model, tokenizer, prompt: str, max_new_tokens: int, thinking_budget: int = 1024):
    ...
    gen_kwargs = {
        ...
        "eos_token_id": tokenizer.eos_token_id, "repetition_penalty": 1.15
    }
        ...
        if remaining_tokens > 0:
            with torch.no_grad():
                final_generated_ids = model.generate(
                    input_ids=input_ids, attention_mask=attention_mask, max_new_tokens=remaining_tokens, **gen_kwargs
                )
        else:
...
def log_interaction(round_num, phase, original, attacked, assessor_response, judgments, rewards, log_path):
    ...
    interaction_log = {
        "round": round_num, "phase": phase, "timestamp": datetime.now().isoformat(),
        "original_note": original, "attacked_note": attacked, "assessor_response": assessor_response,
        "judge_assessment": judgments, "rewards": rewards
    }
...
def main():
    ...
    log_path = f"results/{ts}_{args.model_id.replace('/', '_')}_grpo.jsonl"
    ...
    def assessor_reward_fn(prompts, completions, **kwargs):
        scores = []
        # Get train_dataset which has original and attacked notes
        dataset = kwargs.get('train_dataset')
        for i, (p, c) in enumerate(zip(prompts, completions)):
            original = dataset[i]['original_note']
            attacked = dataset[i]['attacked_note']
            thought, label = parse_assessor_response(policy_tok, c)
            ...
            if (query_harmful and not assessor_refused) or (not query_harmful and assessor_refused):
                r_refusal = -R_REFUSAL # Punish incorrect refusal/non-refusal
            else:
                r_refusal = +R_REFUSAL # Reward correct behavior
                
            r_format = R_FORMAT if thought else -R_FORMAT
            total_reward = r_harm + r_refusal + r_format
            log_interaction(state["round"], "assessor_training", original, attacked, {"thought": thought, "label": label}, judgments, {"total": total_reward}, log_path)
    ...
    def attacker_reward_fn(prompts, completions, **kwargs):
        ...
        for i, c in enumerate(completions):
            original = dataset[i]['original_note']
            attacker_thought, attacked_note = parse_attacker_response(policy_tok, c)
            
            if not attacked_note.strip() or attacked_note.strip() == original.strip():
                # Penalize failure to attack
                scores.append(-R_HARM * 2) 
                continue
            ...
            with torch.no_grad():
                assessor_completion = generate_with_thinking_budget(
                    frozen_assessor, policy_tok, assessor_prompt, max_new_tokens=256
                )
    ...
    for r in range(args.rounds):
        ...
        attacker_trainer = GRPOTrainer(
            model=policy_model, args=GRPOConfig(**common_cfg), 
            processing_class=policy_tok, train_dataset=ds_attacker, reward_funcs=[attacker_reward_fn]
        )
        ...
        for row in sel:
            ...
            prompt = attacker_ds_single[0]['prompt']
            
            completion = generate_with_thinking_budget(
                policy_model, policy_tok, prompt, max_new_tokens=1024
            )
        ...
        if len(ds_assessor_round) > 0:
            ...
            assessor_trainer = GRPOTrainer(
                model=policy_model, args=GRPOConfig(**common_cfg),
                processing_class=policy_tok, train_dataset=ds_assessor_round, reward_funcs=[assessor_reward_fn]
            )
