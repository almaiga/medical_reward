...
def generate_with_thinking_budget(model, tokenizer, prompt: str, max_new_tokens: int, thinking_budget: int = 1024):
    ...
    gen_kwargs = {
        ...
        "repetition_penalty": 1.15 # Slightly increased to discourage loops
    }

    # 1. First generation up to the thinking budget
    with torch.no_grad():
        generated_ids = model.generate(**model_inputs, max_new_tokens=thinking_budget, **gen_kwargs)
    
    output_ids = generated_ids[0, input_length:].tolist()
    
    # Check if the generation has already finished OR if the thinking process has finished
    if IM_END_TOKEN_ID not in output_ids and THINK_END_TOKEN_ID not in output_ids:
        ...
        # 2. Append the early-stopping prompt as per the documentation
        ...
        # Combine the first generation with the stopping prompt
        input_ids = torch.cat([generated_ids, early_stopping_ids], dim=-1)
        attention_mask = torch.ones_like(input_ids)

        # 3. Second generation to get the final content
        ...
        # Thinking finished within the budget, or the whole response finished
...
def test_logic_flow(model, tokenizer, num_examples=3):
    """Test the complete logic flow using Qwen3's native thinking with the budget method."""
    ...
    for i, example in enumerate(examples):
        ...
        # The total max tokens should accommodate the note, thought, and early stopping prompt
...
def main():
    ...
    # Recommend using the official model for baseline testing
