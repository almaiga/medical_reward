...
def parse_attacker_response(tokenizer, full_output_text: str):
    ...
    note_block_match = re.search(
        r'\*\*(?:Modified|Altered|Revised)?\s?Note:\*\*(.*)', 
        final_content, 
...
def get_judge_assessment(original: str, attacked: str, assessor_label: str, judge_model, judge_tok, device):
    ...
    system_prompt = (
        ...
        "   - **Concerning**: Minor error that could delay diagnosis or cause risk.\n"
        "   - **Harmful**: Clear error that could lead to incorrect treatment or significant harm.\n"
    ...
    if label == judgments["actual_harm"]:
...
def make_assessor_prompts(records: list, tokenizer):
    ...
    for rec in records:
        ...
        prompt_string = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, enable_thinking=True)
        tokenized = tokenizer(prompt_string, truncation=True, max_length=2048)
        prompts.append({"input_ids": tokenized["input_ids"], "attention_mask": tokenized["attention_mask"], "prompt": prompt_string, "original_note": rec['original'], "attacked_note": rec['attacked']})
...
def main():
    ...
    log_path = f"results/{ts}_{args.model_id.replace('/', '_')}_grpo_zerosum.jsonl"
    ...
    def assessor_reward_fn(prompts, completions, **kwargs):
        ...
        for i, c in enumerate(completions):
            ...
            _, label = parse_assessor_response(policy_tok, c) # Thought is not needed for reward
            judgments = get_judge_assessment(original, attacked, label, judge_model, judge_tok, device)
            
            # Binary, zero-sum reward
    ...
    def attacker_reward_fn(prompts, completions, **kwargs):
        ...
        for i, c in enumerate(completions):
            ...
            if not attacked_note.strip() or attacked_note.strip() == original.strip():
                # Failed attack is an easy win for the assessor, so attacker gets a penalty implicitly.
                # The assessor will correctly label the note "Safe", getting +1, so attacker gets -1.
                # We can simulate this without running the models.
                ...
                with torch.no_grad():
                    assessor_completion = generate_with_thinking_budget(frozen_assessor, policy_tok, assessor_prompt, max_new_tokens=256)
                ...
                # Binary, zero-sum reward (inverse of assessor's)
                total_reward = -1.0 if judgments["assessor_was_correct"] else 1.0
    ...
    for r in range(args.rounds):
        ...
        for row in sel:
            ...
            completion = generate_with_thinking_budget(policy_model, policy_tok, prompt, max_new_tokens=512)
